...
module.exports = World = cls.Class.extend({
  init: function(id, maxPlayers, websocketServer, databaseHandler, logHandler) {
    var self = this;
      ...
      self.forEachCharacter(function(character) {
        if(character.type === 'player') {
          if(!character.hasFullHealth()) {
            var amount = character.maxHitPoints / 15;
            if(character.pendantSkillKind == Types.Skills.RECOVERHEALTH) {
              amount += character.maxHitPoints * (character.pendantSkillLevel * 0.004);
            }
            if(character.ringSkillKind == Types.Skills.RECOVERHEALTH) {
              amount += character.maxHitPoints * (character.ringSkillLevel * 0.004);
            }
            character.regenHealthBy(Math.floor(amount));
            self.pushToPlayer(character, character.regen());
          }

          if(!character.hasFullMana()){
            character.regenManaBy(Math.ceil(character.level*0.05));
            self.pushToPlayer(character, new Messages.Mana(character));
          }
        } else if(!character.hasFullHealth()){
          character.regenHealthBy(Math.floor(character.maxHitPoints / 20));
        }
      });
    });
  },
  ...
  provocateMob: function(player, mob){
    mob.forgetHater();
    this.handleMobHate(mob.id, player.id, player.level*12); // FIXME
  },
  ...
  handleHurtEntity: function(entity, attacker, damage) {
    ...
    if(entity.hitPoints <= 0) {
      if(entity.type === "mob") {
        ...
        var exp = Types.getMobExp(mob.kind);

        var player = mainTanker && mainTanker instanceof Player ? mainTanker : attacker;

        if(player.party){
          player.party.incExp(mob, this.logHandler);
        } else{
          if(player.pendantSkillKind == Types.Skills.ADDEXPERIENCE) {
            exp = Math.floor(exp + (exp * (player.pendantSkillLevel * 0.005)));
          }
          player.incExp(exp);
          this.logHandler.addExpLog(player, 'kill', mob, exp);
        }
  ...
  pushKungWord: function(player, word){
    ...
    this.lastKungLetters += word[2];
    if(this.lastKungLetters.length > 10){
      this.lastKungLetters = this.lastKungLetters.slice(1);
    }
    ...
    this.kungTimeCallback = setTimeout(function(){
      ...
      if(self.lastKungPlayer && self.kungWords.length >= 10){
        var count = Math.floor(self.kungWords.length/2);
        self.lastKungPlayer.inventory.putInventory(Types.Entities.BURGER, count, 0, 0);
        self.pushBroadcast(new Messages.Kung(self.lastKungPlayer.name + ' : +' + count + ' burgers'));
      }
      self.lastKungPlayer = null;
      self.kungWords = [];
      self.kungedPlayers = [];
      self.lastKungLetters = '';
      self.kungTimeCallback = null;
    }, 10000);
  },
  checkKungPlayer: function(player){
    var i = 0;
    if(player === this.lastKungPlayer){
      player.send([Types.Messages.NOTIFY, "연속으로 하실 수 없습니다."]);
      return false;
    }

    if(this.kungWords.length > 10){
      for(i=0; i<this.kungedPlayers.length; i++){
        if(player === this.kungedPlayers[i]){
          return true;
        }
      }
      player.send([Types.Messages.NOTIFY, "처음 10번 안에 참가한 사람만 할 수 있습니다."]);
      return false;
    } else{
      this.kungedPlayers.push(player);
      return true;
    }
  },
  isAlreadyKung: function(word, player){
    var i=0;
    for(i=0; i<this.kungWords.length; i++){
      if(this.kungWords[i] === word){
        player.send([Types.Messages.NOTIFY, "이미 나온 단어입니다."]);
        return true;
      }
    }
    return false;
  },
  isRightKungWord: function(word, player){
    if(this.kungWords.length === 0){
      return true;
    }

    var i=0;
    for(i=0; i<this.lastKungLetters.length; i++){
      if(this.lastKungLetters[i] === word[2]){
        player.send([Types.Messages.NOTIFY, "최근 10번 안에 나온 단어와 끝글자가 같습니다."]);
        return false;
      }
    }
