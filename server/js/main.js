...
function main(config) {
  ...
  log.info("Starting BBO game server...");
  ...
  server.onConnect(function(connection) {
    var world, // the one in which the player will be spawned
        connect = function() {
          if(world) {
            world.connect_callback(new Player(connection, world, databaseHandler, logHandler));
          }
        };
  ...
  _.each(_.range(config.nb_worlds), function(i) {
    var world = new WorldServer('world'+ (i+1), config.nb_players_per_world, server, databaseHandler, logHandler);
